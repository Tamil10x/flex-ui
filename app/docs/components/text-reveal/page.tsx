    ...
    <div className="min-w-0 flex-1 space-y-14">
      ...
      <DocSection id="what-you-get" title="What You Get">
        <div className="grid gap-3 sm:grid-cols-2">
          {[
            { icon: "~", label: "Dual Reveal Modes", desc: "Choose character-by-character or word-by-word reveal to match the desired pacing and style." },
            { icon: "o", label: "Blur + Slide Animation", desc: "Each unit animates from blurred and offset to sharp and in-place for a cinematic entrance." },
            { icon: "#", label: "Viewport Triggered", desc: "Uses IntersectionObserver via useInView to start the animation when the text scrolls into view." },
            { icon: "+", label: "Reduced Motion Support", desc: "Automatically skips all animations when the user has prefers-reduced-motion enabled." },
          ].map((item) => (
      ...
      <DocSection id="accessibility" title="Accessibility">
        <div className="rounded-xl border border-white/[0.06] bg-zinc-950/50 p-5">
          <ul className="space-y-3 text-sm text-zinc-400">
            {["Renders a plain text fallback when prefers-reduced-motion is enabled, ensuring content is always visible.", "The full text content is present in the DOM from render, so screen readers can announce it immediately.", "Animation is purely visual — the text meaning and structure are unaffected by the reveal effect."].map((note, i) => (
